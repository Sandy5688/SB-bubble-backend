...
const signUp = async (req, res) => {
  try {
    ...
    const tokens = await generateTokenPair(user, { ipAddress: req.ip, userAgent: req.get('user-agent') });
    logger.info('User registered', { userId: user.id, email });
    res.status(201).json({ success: true, data: { user: { id: user.id, email: user.email }, tokens } });
  } catch (error) {
